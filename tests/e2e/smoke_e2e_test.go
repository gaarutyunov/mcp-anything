...
`
	...
	smokeHTTPUpstreamYAML = `apiVersion: mcp-anything.ai/v1alpha1
kind: MCPUpstream
metadata:
  name: smoke-http
  namespace: smoke-e2e
  labels:
    mcp-anything.ai/proxy: smoke-proxy
spec:
  type: http
  toolPrefix: http
  serviceRef:
...
`
	...
	smokeCmdUpstreamYAML = `apiVersion: mcp-anything.ai/v1alpha1
kind: MCPUpstream
metadata:
  name: smoke-cmd
  namespace: smoke-e2e
...
//
// The test installs the mcp-anything operator via its Helm chart, then creates the
...
// helmInstallOperator installs the mcp-anything operator Helm chart into the shared
