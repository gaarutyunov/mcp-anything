...
//
// This test confirms that the CRD YAML files (generated by cmd/crdgen) correctly
// describe the proxy configuration and that the operator reconciles them into a
...
func TestHelmChartSmokeE2E(t *testing.T) {
	...
	// ── 6. Create OpenAPI spec ConfigMap ──────────────────────────────────────
	...
	// ── 7. Start operator in-process ─────────────────────────────────────────

	t.Log("starting operator in-process")
	stopOperator := startOperator(ctx, t, globalK3s.kubeConfigYAML, scheme)
	defer stopOperator()

	// ── 8. Create HTTP MCPUpstream (ServiceRef + ConfigMap OpenAPI) ───────────

	t.Logf("creating HTTP MCPUpstream %s/%s", smokeNamespace, smokeHTTPUpstream)
	httpUpstream := &v1alpha1.MCPUpstream{
		ObjectMeta: metav1.ObjectMeta{
			Name:      smokeHTTPUpstream,
			Namespace: smokeNamespace,
			Labels: map[string]string{
				"mcp-anything.ai/proxy": smokeProxyName,
			},
		},
		Spec: v1alpha1.MCPUpstreamSpec{
			Type:       "http",
			ToolPrefix: "http",
			ServiceRef: &v1alpha1.ServiceRefSpec{
				Name: "wiremock",
				Port: wiremockPort,
			},
			...
			Validation: &v1alpha1.MCPUpstreamValidationSpec{
				ValidateRequest:  true,
				ValidateResponse: false,
			},
		},
	}
	if err := k8sClient.Create(ctx, httpUpstream); err != nil && !apierrors.IsAlreadyExists(err) {
		t.Fatalf("creating HTTP MCPUpstream: %v", err)
	}

	// ── 9. Create Command MCPUpstream ────────────────────────────────────────

	t.Logf("creating Command MCPUpstream %s/%s", smokeNamespace, smokeCmdUpstream)
	cmdUpstream := buildSmokeCommandUpstream()
	if err := k8sClient.Create(ctx, cmdUpstream); err != nil && !apierrors.IsAlreadyExists(err) {
		t.Fatalf("creating Command MCPUpstream: %v", err)
	}

	// ── 10. Create MCPProxy ───────────────────────────────────────────────────

	t.Logf("creating MCPProxy %s/%s", smokeNamespace, smokeProxyName)
...
// buildSmokeCommandUpstream constructs the command MCPUpstream for the smoke test.
func buildSmokeCommandUpstream() *v1alpha1.MCPUpstream {
	return &v1alpha1.MCPUpstream{
		ObjectMeta: metav1.ObjectMeta{
			Name:      smokeCmdUpstream,
			Namespace: smokeNamespace,
			Labels: map[string]string{
				"mcp-anything.ai/proxy": smokeProxyName,
			},
			...
			Commands: []v1alpha1.MCPUpstreamCommandSpec{
				{
					...
					Shell:       true,
					Timeout:     "5s",
					Env:         map[string]string{"SMOKE_ENV": "smoke-test-value"},
