// Package command implements command-backed MCP tool execution.
// It provides the Def type for shell command execution, and BuildTools for
// converting CommandConfig entries into RegistryEntry-compatible tool descriptors.
package command
...
// upstream.New() to build RegistryEntry objects.
type Tool struct {
	PrefixedName string
	OriginalName string
	MCPTool      *sdkmcp.Tool
